...
**** to include somewhere [discussion]. . . *in manuscript*
...
**** new supp figure for Tau sensitivity of phase shifts - *in manuscript*
...
**** new paragraph looking at high-subthreshold phase shifts in 3 different cell models and 3 morphologies with the same biophysics. - *in manuscript*
High amplitude subthreshold chirp stimuli produce different frequency-dependent 
phase shifts for depolarization and hyperpolarization in three different models
of cortical pyramidal neurons \fref{compareModelsSubthresh}A. \textbf{Model 1} and
\textbf{model 2} are both models of L5b pyramidal neurons with different morphologies 
and channel distributions.  \textbf{Model 2} exhibited lower synchronous frequency and 
less phase lead for hyperpolization and no synchrony or phase lead for depolarization.
\textf{Model 3}, a model of CA1 pyramidal cell, exhibited phase lead and synchrony for
depolarization and hyperpolarization, like \textbf{model 1}, but there was more phase lead 
and higher synchrony point for depolarization than hyperpolarization, unlike in the models 
of neocortical pyramidal neurons. We also investigated the role of morphology by stimulating 
models of L5b pyramidal neurons with different morphologies but the same channel distributions 
used in \textbf{model 1} \fref{compareModelsSubthresh}B.  While morphology affected frequency dependent phase shifts 
for depolarization and hyperpolarization, it had a minor affect compared to the differences among 
the three models mentioned above.
**** new figure 
\caption{{\bf Different models of cortical pyramidal neurons exhibited different phase shifts for depolarization and hyperpolarization.}
    A. Morphologies (left) of three models of cortical pyramidal neurons (two from L5b, one from CA1) and their frequency dependent phase 
    shifts for depolarization (dashed lines) and hyperpolarization (solid) in response to high amplitude subthreshold chirp stimuli (right). 
    Each cell model has a unique channel distribution.
    Phase shifts for \textbf{model 1} are reproduced from \fref{sub_thru_spk}B.
    B. Morphologies (left) of three L5b pyramidal neuron models with the same channel distributions (from \textbf{model 1}) and their frequency dependent
    phase shifts for depolarization (dashed lines) and hyperpolarization (solid) in response to high amplitude subthreshold chirp stimuli (right).
    Morphology 1 is the same morphology used in \textbf{model 1}.}
**** new paragraph on CA1 channels / inductance - *in manuscript*
The situation was even more complex in the model of CA1 pyramidal neuron (\textbf{model 3}), 
In this cell, phenomenological inductance was generated by both \ih\ and \im\ (\fref{mmCa1}).
Unlike the two L5b pyramidal neuron models, \textbf{model 3} showed higher $\Phi^+$ than $\Phi^-$.
Blocking \ih\ elimated synchrony and phase lead for hyperpolarization but not for depolarization. 
This is because \im\ generates it's own ``perithreshold'' phenomenological inductance \cite{DAngelo1998-qx, DAngelo2001-zh, Izhikevich2002-df, Vera2017-ne}, which produced resonance and here selectively increases phenomenological inductance 
for high amplitude depolarizations.
Blocking \im\ perserved the \ih\ dependent phenomenological inductance producing phase lead for hyperpolarization and produced phase lag for depolarization across stimulus frequencies.
Blocking both \ih\ and \im, eleiminated phase lead and synchrony for both depolarization and hyperpolarization, but there remained a large separation between $\Phi^+$ and $\Phi^-$.
This separation was narrowed by also blocking \ia.
**** new figure 
\caption{{\bf Multiple resonant currents can contribute to phenomenological inductance governing phase shifts for depolarization and hyperpolarization.}
    Phase shifts in \textbf{model 3} (CA1 pyramidal neuron) in response to high amplitude subthreshold chirp stimuli under control conditions (black) 
    and when blocking a number of currents.  Blocking \ih\ eliminated phase lead and synchrony for hyperpolarization but preserved \im\ dependent phenomenological
    inductance that produced phase lead and synchrony for depolarization.  Blocking \im\ preserved phase lead and synchrony for hyperpolarization, not depolarization.
    Blocking both currents elimated phenomenological inductance. Blocking both and \ia\ reduced differences between $\Phi^+$ and $\Phi^-$.}
...
for sec in h.allsec():
    for seg in sec.allseg():
        try:
            ...
            pass
*** similar to M1 cell 
** code to plot dots on traces 
plt.plot(time, soma_v, 'b')
plt.plot([time[pk] for pk in pks], [soma_v[pk] for pk in pks], 'b*')
plt.plot([time[pk] for pk in trghs], [soma_v[pk] for pk in trghs], 'bo')
plt.plot(time, (i.as_numpy()*13-70), 'k')
plt.plot([time[pk] for pk in itrghs], [i[pk]*13-70 for pk in itrghs], 'ko')
plt.plot([time[pk] for pk in ipks], [i[pk]*13-70 for pk in ipks], 'k*')

* [2023-6-22 Thu] - mm cell 
** depol higher phi than hyperpol (opposite other models)
** running block of all the other channels 
** plotting channels 
current_names = ['Ih', 'TASK', 'Na3', 'Ka', 'Kdr']
config = {
        "output": {
            "savefig": True,
            "dir": ".",
...
* [2023-6-23 Fri] - more ca1 cell
** figured out Ih issues and cause of flipped Phi+/- (separate Im resonance)
** no phase retreat (believe no ahp current)
** strong phase advance 

* [2023-6-24 Sat] - plan for new figs/models 
** outline for figures 
*** 1, 2, unchanged 
*** DONE - new 3 - compare three models with morphs and phase shifts at baseline
*** 3->4  
*** current 4 might get consolidated 
*** DONE - new 5 - ca1 model and multiple resonances 
*** 5->7, DONE - replace amplitude with other models 
*** 6->8, maybe not - include comparison for ca1 cell 
*** 7->9, DONE - replace amplitude with other models 

* [2023-6-28 Wed] - figs 
** most changes coded but not put into proper figs and into manuscript 

* [2023-7-17 Mon] - alternative titles 
** base: Membrane potential phase shifts differ for excitation vs. inhibition in theta-resonant pyramidal neurons: a computer modeling study
*** Exploring nonlinear membrane dynamics in computer models of theta-resonant pyramidal neurons: different phase shifts for excitation vs. inhibition and nonstationary spiking patterns 
*** Membrane potential phase shifts differ for excitation vs. inhibition in computer models of theta-resonant pyramidal neurons

* TODOS 
** DONE: graphical abstract
** DONE: fix references  
** DONE: decide whether to include ... in description of motivation to use the model:
...
*** maybe not
** DONE: consider suprathreshold chirp for ca1 cell
*** decided no 
** DONE: clinical relevance of Iahp
** DONE: ca1 suprathreshold block Im
** DONE: add bit on amplitude vs time 
** DONE: how to incorporate CA1 results 
** DONE: check asymmetry of apical responses
** DONE: intro para 1 focus on l5b
** DONE: update methods on other two models 
